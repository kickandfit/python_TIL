...
ols1.pvalues # p-value
ols1.params # 절편 및 회귀계수

# 유의미한 변수 수, 즉 귀무가설 기각한 수
(ols1.pvalues < 0.05).sum() - 1
# 13

# 유의미한 변수 중 회귀계수가 음수인 변수 수
...
q1=data7[x_var].corr().drop('Chance_of_Admit')

q1['Chance_of_Admit'].nlargest(1)

# (정답) CGPA    0.873289 -> 0.873
...
logit.fit(q3[x_var], q3['ch_cd'])

q3_out=pd.DataFrame({'var': x_var,
                     'coef':abs(logit.coef_).reshape(-1)})

q3_out['coef'].nlargest(1)

q3_out.sort_values('coef', ascending=False)

# (정답)  CGPA,  1.978584-> 1.98
...
# (1) 이전 소유자 수가 한 명이고 엔진 파워가 51인 차 추출
q1=data10[data10.previous_owners == 1]
q1=q1[q1.engine_power == 51]

# q1=data10[(data10.previous_owners == 1) & (q1.engine_power == 51)]

# (2) 모델별 평균: 운행 일수에 대해서도 평균을 산출해야 함
...
# Ttest_indResult(statistic=1.5054268543179226, 
#                 pvalue=0.13248244438755083)
q2_out.pvalue < 0.05
